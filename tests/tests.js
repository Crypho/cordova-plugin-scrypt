...
exports.defineAutoTests = function() {
  describe('Scrypt (window.plugins.scrypt)', function () {
...
exports.defineManualTests = function(contentEl, createActionButton) {
  var logMessage = function (message, color) {
    var log = document.getElementById("scrypt_log");
  ...
  var updatePassed = function(cnt){
    var passed = document.getElementById("passed_cnt");
    passed.style.color = "green";
    passed.innerHTML = cnt;
  }

  var updateFailed = function(cnt){
    var failed = document.getElementById("failed_cnt");
    failed.style.color = "red";
    failed.innerHTML = cnt;
  }
  ...
  contentEl.innerHTML = '<div id="info"><div id="scrypt_log"></div></div> <div><strong>Passed Tests: <span id="passed_cnt">0</span><br/> Failed Tests: <span id="failed_cnt">0</span></strong></div><div id="check_scrypt"></div>';

  var check_testCase = function (successCallback, errorCallback, testCase){
    var errors = 0;
    var expected = testCase["expected"];
    var password = testCase["password"];
    var salt = testCase["salt"];
    var N = testCase["N"];
    var r = testCase["r"];
    var p = testCase["p"];
    var dkLen = testCase["dkLen"];
    window.plugins.scrypt(
      function(res){
        if (res != expected) {
          ...
          logMessage("--- Scrypt failed", "red");
          logMessage("Params: " + JSON.stringify(params, null, '\t'));
      ...
      function(res){
        logMessage("Scrypt failed:" + res, "red");
        errorCallback();
      },
      password,
      salt,
  ...
  createActionButton('Run Test Cases', function() {
    clearLog();
    logMessage("Running Test Cases", "blue");
    var arrayLength = testCases.length;
    var errors = 0;
    var passed = 0;
    var check_finish = function() {
      if (errors + passed == arrayLength) {
        logMessage("Finished", "blue");
      }
    }

    for (var i = 0; i < arrayLength; i++) {
      var options = testCases[i];
      check_testCase(
        function () {
          passed += 1;
          updatePassed(passed);
